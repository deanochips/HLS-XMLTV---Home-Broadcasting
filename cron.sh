...
function generate_m3u {
	...
	do
		vars=( $args )
                echo '#EXTINF:0 channelID="x-ID.'${vars[1]:1:-1}.tv'" tvg-chno="'$((${vars[1]:1:-1} + 1000))'" tvg-name="'${vars[0]:1:-1}'" tvg-id="'${vars[0]:1:-1}'.tv" tvg-logo="" group-title="",'${vars[0]:1:-1}''  >> "$M3U_DIR"/"streams.m3u"
